function Draw_MIPG(vhist,zhist,vp1hist,zp1hist,vp2hist,zp2hist,vp3hist,zp3hist,dhist,T,N)
...
plot_car(an,alt,x_4(k),y_4(k),0,'m')%Plot the car
hold on;
...
%------------agente 4------------
    plot(x_4(k),y_4(k),'-r','linewidth',line_width);hold on % plot exhibited trajectory
    if k < size(vhist,2) % plot prediction
        plot(xp_4(k,1:N),zph_4(k,1:N),'m--*')
    end
    plot(x_4(k)+xp,y_4(k)+yp,'--r')% plot robot circle 
