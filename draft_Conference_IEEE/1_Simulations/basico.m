...
Vdes=[30; 10; 10]; %velocidad deseada

zel= [2; 3; 5]; %carril inicial
Zdes=[4; 3; 5]; %carril deseado
...
% parameters_in = { Vd , Zd , v{1} , p_z , ...
%                             v_2 , z_2 , dis12{1} , Aa1 , Bb1 , Ss1 , Nn1};%, Gg1
% solutions_out = {[a{:}], [z{:}], [v{:}], [a1{:}],  [b1{:}] , [g1{:}], [s1{:}], [n1{:}]};
...
toc
vp2hist = [vel(2)*ones(time,N+1)];
zp2hist = [zel(2)*ones(time,N+1)];
vp3hist = [vel(3)*ones(time,N+1)];
zp3hist = [zel(3)*ones(time,N+1)];
