...
constraints = [constraints, [sum(N1,1,i)==1], 
              implies( N1(1,1,i), [ n1==0,       dis12{1} <= -Dl]);
              implies( N1(2,1,i), [ n1==1, -Dl<= dis12{1} <= Dl ]);
              implies( N1(3,1,i), [ n1==0,  Dl<= dis12{1}]) ];   
%................................................................................
...
constraints = [constraints, [D2(1,1,i)+D2(2,1,i)+D2(3,1,i)==1], 
              implies( D2(1,1,i), [ a2==0, z_3-z{k} <=-0.1 ]);
              implies( D2(2,1,i), [ a2==1, -0.1<=z_3-z{k} <=0.1 ]);
              implies( D2(3,1,i), [ a2==0, 0.1 <= z_3-z{k} ]) ];
%.........................Beta...............................
constraints = [constraints, [sum(B2,1,i)==1], 
              implies(B2(1,1,i),[ b2==1, dis13{1} >=0 ]);
              implies(B2(2,1,i),[ b2==0, dis13{1} <=0 ]) ];
          
constraints = [constraints, [dis13{1}<=100000]]; 

% %.........................Gamma...............................
constraints = [constraints, [G2(1,1,i)+G2(2,1,i)+G2(3,1,i)==1], 
              implies( G2(1,1,i), [ g2==0, z_3-z{k+1} <=-0.1 ]);
              implies( G2(2,1,i), [ g2==1, -0.1<=z_3-z{k+1} <=0.1 ]);
              implies( G2(3,1,i), [ g2==0, 0.1 <= z_3-z{k+1} ]) ];   
...
constraints = [constraints, [sum(N2,1,i)==1], 
              implies( N2(1,1,i), [ n2==0,       dis13{1} <= -Dl]);
              implies( N2(2,1,i), [ n2==1, -Dl<= dis13{1} <= Dl ]);
              implies( N2(3,1,i), [ n2==0,  Dl<= dis13{1}]) ];   
%................................................................................
...
constraints = [constraints, [sum(B3,1,i)==1], 
              implies(B3(1,1,i),[ b3==1, dis14{1} >=0 ]);
              implies(B3(2,1,i),[ b3==0, dis14{1} <=0 ]) ];
          
constraints = [constraints, [dis14{1}<=100000]]; 

% %.........................Gamma...............................
constraints = [constraints, [G3(1,1,i)+G3(2,1,i)+G3(3,1,i)==1],
              implies( G3(1,1,i), [ g3==0, z_4-z{k+1} <=-0.1 ]);
              implies( G3(2,1,i), [ g3==1, -0.1<=z_4-z{k+1} <=0.1 ]);
              implies( G3(3,1,i), [ g3==0, 0.1 <= z_4-z{k+1} ]) ];   
          
constraints = [constraints, [1<=z{k+1}<=L]];           

% %.........................Lateral distance...............................
constraints = [constraints, [sum(Z3,1,i)==1], 
              implies( Z3(1,1,i), [ z3==0,       z_4-z{k} <= -1.1 ]);
              implies( Z3(2,1,i), [ z3==1, -1.1<=z_4-z{k} <= -0.9 ]);
              implies( Z3(3,1,i), [ z3==0, -0.9<=z_4-z{k} <= 0.9 ]);
              implies( Z3(4,1,i), [ z3==1,  0.9<=z_4-z{k} <= 1.1 ]);
              implies( Z3(5,1,i), [ z3==0,            1.1 <= z_4-z{k} ]) ];   
constraints = [constraints, [1<=p_z<=L]];

% %.........................lateral safety distance...............................
constraints = [constraints, [sum(N3,1,i)==1], 
              implies( N3(1,1,i), [ n3==0,       dis14{1} <= -Dl]);
              implies( N3(2,1,i), [ n3==1, -Dl<= dis14{1} <= Dl ]);
              implies( N3(3,1,i), [ n3==0,  Dl<= dis14{1}]) ];   
%................................................................................
...
%     g1 = solutions1{5};  %g1hist=[g1hist g1];
    B1 = solutions1{5};  b1hist=[b1hist B1];     blogic1_1=B1(:,1);
    Gg1 = solutions1{6}; g1hist=[g1hist Gg1];    G1logic_1=Gg1(:,1);
    Z1 = solutions1{7};  z1hist=[z1hist Z1];     S1logic_1=Z1(:,1);
    N1 = solutions1{8};  n1hist=[n1hist N1];     N1logic_1=N1(:,1);
...
%     g3 = solutions1{17};  %g1hist=[g1hist g1];
    B3 = solutions1{15};  b3hist=[b3hist B3];     blogic3_1=B3(:,1);
    Gg3 = solutions1{16}; g3hist=[g3hist Gg3];    G3logic_1=Gg3(:,1);
