...
%------------------si dz=0  -------------------->>>    dij>= Ds----------------
    ...
    constraints = [constraints, dis13{k+1}(i) == dis13{k}(i)+T*(v_3(i)-v{k})];
%.........................alpha...............................
...
constraints = [constraints,  Aa2(i)*Gg2(i)*(-Bb2(i)*(T*(v_3(i)-v{k})+dis13{k}(i))+(1-Bb2(i))*(T*(v_3(i)-v{k})+dis13{k}(i)))<=0];
...
%------------------si dz=0  -------------------->>>    dij>= Ds----------------
    ...
    constraints = [constraints, -10000  <=  dis14{k+1}(i) <= 100000,
                                  mmin  <= z_4(i)-z{k+1}  <= Mmax];
    constraints = [constraints, dis14{k+1}(i) == dis14{k}(i)+T*(v_4(i)-v{k})];
%.........................alpha...............................
constraints = [constraints, [D3(1,1)+D3(2,1)+D3(3,1)==1], 
              implies( D3(1,1), [ a3==0, z_4-z{k} <=-0.2 ]);
              implies( D3(2,1), [ a3==1, -0.2<=z_4-z{k} <=0.2 ]);
              implies( D3(3,1), [ a3==0, 0.2 <= z_4-z{k} ]) ];
%.........................Beta...............................
constraints = [constraints, [sum(B3,1,i)==1], 
              implies(B3(1,1,i),[ b3(i)==1, dis14{1}(i) >=0 ]);
              implies(B3(2,1,i),[ b3(i)==0, dis14{1}(i) <=0 ]) ];
          
constraints = [constraints, [dis14{1}(i)<=100000]]; 

% %.........................Gamma...............................
constraints = [constraints, [G3(1,1,i)+G3(2,1,i)+G3(3,1,i)==1], 
              implies( G3(1,1,i), [ g3(i)==0, z_4(i)-z{k+1} <=-0.1 ]);
              implies( G3(2,1,i), [ g3(i)==1, -0.1<=z_4(i)-z{k+1} <=0.2 ]);
              implies( G3(3,1,i), [ g3(i)==0, 0.1 <= z_4(i)-z{k+1} ]) ];   
          
constraints = [constraints, [1<=z{k+1}<=L]];           

% %.........................Lateral distance...............................
constraints = [constraints, [sum(Z3,1,i)==1], 
              implies( Z3(1,1,i), [ z3(i)==0,       z_4(i)-z{k} <= -1.1 ]);
              implies( Z3(2,1,i), [ z3(i)==1, -1.1<=z_4(i)-z{k} <= -0.9 ]);
              implies( Z3(3,1,i), [ z3(i)==0, -0.9<=z_4(i)-z{k} <= 0.9 ]);
              implies( Z3(4,1,i), [ z3(i)==1,  0.9<=z_4(i)-z{k} <= 1.1 ]);
              implies( Z3(5,1,i), [ z3(i)==0,            1.1 <= z_4(i)-z{k} ]) ];   
constraints = [constraints, [1<=p_z<=L]];

% %.........................lateral safety distance...............................
constraints = [constraints, [sum(N3,1,i)==1], 
              implies( N3(1,1,i), [ n3(i)==0,       dis14{1}(i) <= -Dl]);
              implies( N3(2,1,i), [ n3(i)==1, -Dl<= dis14{1}(i) <= Dl ]);
              implies( N3(3,1,i), [ n3(i)==0,  Dl<= dis14{1}(i)]) ];   
%................................................................................
constraints = [constraints,  Aa3(i)*(Bb3(i)*(Ds - dis14{k+1}(i))+(1-Bb3(i))*(Ds + dis14{k+1}(i)))<=0];
constraints = [constraints,  Aa3(i)*Gg3(i)*(-Bb3(i)*(T*(v_4(i)-v{k})+dis14{k}(i))+(1-Bb3(i))*(T*(v_4(i)-v{k})+dis14{k}(i)))<=0];
constraints = [constraints, Ss3(i)*(Nn3(i))*(z{k}-p_z)==0];

% ------------------------------------vehiculo 5-------------------------------    
%------------------si dz=0  -------------------->>>    dij>= Ds----------------

    constraints = [constraints, -10000  <=  dis15{k+1}(i) <= 100000,
                                  mmin  <= z_5(i)-z{k+1}  <= Mmax];
    constraints = [constraints, dis15{k+1}(i) == dis15{k}(i)+T*(v_5(i)-v{k})];
%.........................alpha...............................
constraints = [constraints, [D4(1,1,i)+D4(2,1,i)+D4(3,1,i)==1], 
              implies( D4(1,1,i), [ a4(i)==0, z_5(i)-z{k} <=-0.2 ]);
              implies( D4(2,1,i), [ a4(i)==1, -0.2<=z_5(i)-z{k} <=0.2 ]);
              implies( D4(3,1,i), [ a4(i)==0, 0.2 <= z_5(i)-z{k} ]) ];
%.........................Beta...............................
constraints = [constraints, [sum(B4,1,i)==1], 
              implies(B4(1,1,i),[ b4(i)==1, dis15{1}(i) >=0 ]);
              implies(B4(2,1,i),[ b4(i)==0, dis15{1}(i) <=0 ]) ];
          
constraints = [constraints, [dis15{1}(i)<=100000]]; 

% %.........................Gamma...............................
constraints = [constraints, [G4(1,1,i)+G4(2,1,i)+G4(3,1,i)==1], 
              implies( G4(1,1,i), [ g4(i)==0, z_5(i)-z{k+1} <=-0.1 ]);
              implies( G4(2,1,i), [ g4(i)==1, -0.1<=z_5(i)-z{k+1} <=0.2 ]);
              implies( G4(3,1,i), [ g4(i)==0, 0.1 <= z_5(i)-z{k+1} ]) ];   
          
constraints = [constraints, [1<=z{k+1}<=L]];           

% %.........................Lateral distance...............................
constraints = [constraints, [sum(Z4,1,i)==1], 
              implies( Z4(1,1,i), [ z4(i)==0,       z_5(i)-z{k} <= -1.1 ]);
              implies( Z4(2,1,i), [ z4(i)==1, -1.1<=z_5(i)-z{k} <= -0.9 ]);
              implies( Z4(3,1,i), [ z4(i)==0, -0.9<=z_5(i)-z{k} <= 0.9 ]);
              implies( Z4(4,1,i), [ z4(i)==1,  0.9<=z_5(i)-z{k} <= 1.1 ]);
              implies( Z4(5,1,i), [ z4(i)==0,            1.1 <= z_5(i)-z{k} ]) ];   
constraints = [constraints, [1<=p_z<=L]];

% %.........................lateral safety distance...............................
constraints = [constraints, [sum(N4,1,i)==1], 
              implies( N4(1,1,i), [ n4(i)==0,       dis15{1}(i) <= -Dl]);
              implies( N4(2,1,i), [ n4(i)==1, -Dl<= dis15{1}(i) <= Dl ]);
              implies( N4(3,1,i), [ n4(i)==0,  Dl<= dis15{1}(i)]) ];   
%................................................................................
constraints = [constraints,  Aa4(i)*(Bb4(i)*(Ds - dis15{k+1}(i))+(1-Bb4(i))*(Ds + dis15{k+1}(i)))<=0];
constraints = [constraints,  Aa4(i)*Gg4(i)*(-Bb4(i)*(T*(v_5(i)-v{k})+dis15{k}(i))+(1-Bb4(i))*(T*(v_5(i)-v{k})+dis15{k}(i)))<=0];
constraints = [constraints, Ss4(i)*(Nn4(i))*(z{k}-p_z)==0];
...
vel=[15; 15; 10; 20; 15];% velociodad inicial
...
Vdes=[5; 20; 25; 10; 10]; %velocidad deseada
Zdes=[1; 4; 3; 5; 2];
...
%                                          [a4], [dis15{:}], [b4], [g4], [z4], [n4]};
    inputs = {Vdes(1),Zdes(1),vel(1),past_a(1),zel(1),...
                        ...
                        vel(5),zel(5),d1i(4),alogic4_1,blogic4_1,G4logic_1,S4logic_1,N4logic_1};
    ...
    A3 = solutions1{16};                          alogic3_1=A3(:,1);
    g3 = solutions1{17};  %g1hist=[g1hist g1];
    B3 = solutions1{18};  b3hist=[b3hist B3];     blogic3_1=B3(:,1);
    Gg3 = solutions1{19}; g3hist=[g3hist Gg3];    G3logic_1=Gg3(:,1);
    Z3 = solutions1{20};  z3hist=[z3hist Z3];     S3logic_1=Z3(:,1);
    N3 = solutions1{21};  n3hist=[n3hist N3];     N3logic_1=N3(:,1);
    
    A4 = solutions1{22};                         alogic4_1=A4(:,1);
    g4 = solutions1{23};  g4hist=[g4hist g4];
    B4 = solutions1{24};  b4hist=[b4hist B4];    blogic4_1=B4(:,1);
    Gg4 = solutions1{25}; g4hist=[g4hist Gg4];   G4logic_1=Gg4(:,1);
    Z4 = solutions1{26};  z4hist=[z4hist Z4];    S4logic_1=Z4(:,1);
    N4 = solutions1{27};  n2hist=[n4hist N4];    N4logic_1=N4(:,1); 
